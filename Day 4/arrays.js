...
const inventors = [
  ...
  const people = ['Beck, Glenn', 'Becker, Carl', 'Beckett, Samuel', 'Beddoes, Mick', 
    ...
    const fullNames = inventors.map(inventor => `${inventor.first} ${inventor.last}`);    // using back-tick!
    ...
    const ordered = inventors.sort((a, b) => a.year > b.year ? 1 : -1);         // using ternary operator in JS
    ...
    //     // const links = [...category.querySelectorAll('a')];          // using spread operator (ES6) in JS
    
    // const de = links
    //             .map(link => link.textContent)
    //             .filter(streetName => streetName.includes('de'));
                                                        // Run the above code!, in the Broswer console ONLY! 
